...
def output(hostname, rdtype, myresponse, elapsed, cnames):
    ...
    second_line = hostname_.ljust(
        39) + 'IN'.ljust(5) + rdtype.ljust(5) + '\n\n'
    ...
    if len(cnames) > 0:
        ...
        for cname in cnames:
            forth_line = forth_line + \
                left.ljust(33) + number.ljust(6) + 'IN'.ljust(5) + 'CNAME'.ljust(7) + cname + '\n'
            left = cname
        for answer in answers:
            forth_line = forth_line + \
                left.ljust(33) + number.ljust(6) + 'IN'.ljust(5) + rdtype.ljust(7) + answer + '\n'
        if(len(answers) == 0):
            forth_line = forth_line + "No Answer for domain " + hostname + "\n"
    else:
        for answer in answers:
            forth_line = forth_line + \
                hostname_.ljust(33) + number.ljust(6) + 'IN'.ljust(5) + rdtype.ljust(7) + answer + '\n'
        if(len(answers) == 0):
            forth_line = forth_line + "No Answer for domain " + hostname + "\n"
...
def get_cname_from_rrset(rrset):
    ...
    try:
        ...
        print('Error! Some issue with CNAME: ', e)
...
def get_ip_from_rrset(rrset):
    ...
    try:
        ...
        print('Error! Some issue with IP: ', e)
...
def get_ns_from_authority(response):
    ...
    rrsets = sorted(rrsets)
    return rrsets[0].to_text()
...
def check_hostname(hostname):
    ...
    Return:
            True or False
    '''
    re_domain = r'^(?=.{4,255}$)(([a-zA-Z0-9][a-zA-Z0-9-]{,61}[a-zA-Z0-9]\.)+|([a-zA-Z0-9-]{,61}[a-zA-Z0-9]\.)+)[a-zA-Z0-9]{2,5}.$'
...
def check_response(response, rdtype):
    ...
    try:
        if rdtype == 'A':
            ...
            # return all the CNAMEs when 'dig www.cnn.com NS'
...
def dns_resolver_3(
    ...
    try:
        ...
        final_person = ""
        counter = 0
        for root in list(root_servers.values()):
            try:
                ...
                if "SOA" in str(response.authority[0]):
                    ...
                    name_servers_answered.append(
                        (hostname, "root root root root " + str(root), "Could not get IP"))
                ...
                # if ANSWER section is empty, then keep iterating
                while(len(response.answer) == 0):
                    ...
                    if len(
                            response.additional) > 0:   # use the IP in ADDITIONAL section
                        for rrset in response.additional:
                            ...
                            try:
                                ...
                                response2 = single_iterate(
                                    hostname, rdtype, next_ip, timeout=0.5)
                                response_code.append(
                                    (" ".join(str(response2).split("\n"))))
                                response_array = str(response2).split("\n")
                                response_code_string = response_array[2].split(" ")[
                                    1]

                                if "REFUSED" == response_code_string:
                                    stop_flag = True
                                    name_servers_answered.append(
                                        (hostname, str(rrset), "Could not get IP"))
                                    return ""
                                ...
                                if str(e) == "Timeout":
                                    for one_record in response.additional:
                                        name_servers_answered.append(
                                            (hostname, str(one_record), "Could not get IP"))
                                    return ""
                        ...
                        if check_hostname(ns):
                            ...
                            # add rrset that contains IP of a AUTHORITY to
                            # response
                            response.additional.append(authority_answer)
                        else:

                            return response   # hostname in AUTHORITY is not valid
                if check_response(response, rdtype):  # ip is in the response
                    name_servers_answered.append(
                        (hostname, str(final_person), "IP"))
                    return response
                    ...
                    for rrset in response.answer:
                        ...
                        cnames.append(cname)
                        name_servers_answered.append(
                            (hostname, str(final_person), "CNAME"))
                        return dns_resolver_3(
                            cname,
                            rdtype,
                            cnames,
                            name_servers,
...
def output_sec(hostname, rdtype, response, elapsed, cnames):
    ...
    Args:
            hostname (str): host to be queried
            rdtype (str): type A, NS, or MX
            response (dns.message.Message): reponse from the DNS query
            elapsed (float): time elapsed
            cnames (list): cnames during a dns query
    '''
...
def dns_resolver_sec(hostname, rdtype, cnames):
    ...
    for root in list(root_servers.values()):
        try:
            response = single_iterate(
                hostname, rdtype, root, timeout=0.5, dnssec=True)
            response_dnskey = single_iterate(
                '.', 'DNSKEY', root, timeout=0.5, dnssec=True)
            ...
            while flag == Flag.NO_ANSWER:
                if len(
                    ...
                    for rrset in response.additional:
                        ...
                        try:
                            response2 = single_iterate(
                                hostname, rdtype, next_ip, timeout=0.5, dnssec=True)
                            response_dnskey2 = single_iterate(
                                nextname, 'DNSKEY', next_ip, timeout=0.5, dnssec=True)
                            ...
                            if nextname == 'org.':  # for org. zone, when dnssec=True, DNSKEY response is empty
                                # so I wrote special functions for org.
                                name_key, dnskey = verify_org_dnskey(next_ip)
                                verify_ds(response2, name_key, dnskey)
                                verify_org_zone(dnskey, response)
                            else:
                                name_key, dnskey = verify_dnskey(
                                    response_dnskey2)
                                verify_ds(response2, name_key, dnskey)
                    ...
                    if check_hostname(ns):
                        ...
                        authority_answer = response2.answer[0]
                        # add rrset that contains IP of a AUTHORITY to response
                        response.additional.append(authority_answer)
                    else:
                        return response   # hostname in AUTHORITY is not valid
...
def alias(array):
    if len(array) == 3:
        ...
        if length != 0 and (
                name_servers_answered[length - 1][2] == "IP") and myresponse:
            ...
            with open(thread_number + "trac_domains_resolved", 'a') as file1:
                file1.write(str(name_servers_answered[length - 1][0]) + " " + str(
                    one_name_server[1].split()[4]) + " " + str(answers[0]) + " " + str(hostname) + "\n")
        else:
            global timeout_flag
            real_host = ""
            real_host_domain = ""
            ...
            if len(response_code) > 0:
                message = ""
                message = real_host_domain + " " + \
                    str(response_code[len(response_code) - 1])
                if(timeout_flag):
