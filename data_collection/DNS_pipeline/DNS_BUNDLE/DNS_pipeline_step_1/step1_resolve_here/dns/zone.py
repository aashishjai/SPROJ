...
class Zone(object):
    ...
    def iterkeys(self):
        return list(self.nodes.keys()) # pylint: disable=dict-keys-not-iterating

    def keys(self):
        return list(self.nodes.keys()) # pylint: disable=dict-keys-not-iterating

    def itervalues(self):
        return list(self.nodes.values()) # pylint: disable=dict-values-not-iterating

    def values(self):
        return list(self.nodes.values()) # pylint: disable=dict-values-not-iterating

    def items(self):
        return list(self.nodes.items()) # pylint: disable=dict-items-not-iterating
    ...
    def iterate_rdatasets(self, rdtype=dns.rdatatype.ANY,
        ...
        if isinstance(covers, str):
            covers = dns.rdatatype.from_text(covers)
        for (name, node) in self.items(): # pylint: disable=dict-iter-method
            for rds in node:
    ...
    def to_file(self, f, sorted=True, relativize=True, nl=None):
        ...
        try:
            if sorted:
                ...
                names = iter(self.keys()) # pylint: disable=dict-iter-method
            for n in names:
