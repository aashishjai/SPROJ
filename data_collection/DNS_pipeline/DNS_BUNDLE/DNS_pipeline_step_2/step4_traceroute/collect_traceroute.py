...
def do_icmp_traceroute(dest,payload):
	...
	for ttl in range(1,25):
		...
		ans,unans=sr(IP(dst=dest["IP"], ttl=ttl,id=ttl,flags=2)/ICMP(id=token,seq=ICMP_SEQ)/str(raw_string),timeout=TIMEOUT,verbose=0)
...
def do_http_traceroute_stateless(dest):
	...
	HTTP_request_str = "GET / HTTP/1.1\r\nHost: " + dest["domain"] + "\r\nUser-Agent: Mozilla/5.0 (X11; root; Linux x86_64; rv:59.0) Gecko/20100101 Firefox/59.0\r\nAccept-Language: en-US,en;q=0.5\r\nConnection: keep-alive\r\nUpgrade-Insecure-Requests: 1\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\nAccept-Encoding:gzip, deflate\r\n\r\n"
...
def do_all_traceroutes1(domain_list,payload_list):
	N_THREADS1 = 2
	children_threads=[]
	THREAD_WORKLOAD1 = int(max(len(domain_list)/ N_THREADS1, 1))
...
if __name__ == "__main__":
	...
	THREAD_WORKLOAD = int(max(len(domain_ip_list)/ N_THREADS, 1))
	'''
	input file 
	abc 1.1.1.1 2.2.2.2
