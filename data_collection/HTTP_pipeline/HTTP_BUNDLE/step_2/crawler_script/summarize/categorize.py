...
def get_geo_block(body):
    """
    Returns a tag of geo-blocked if the HTML body contains any regexes
    which indicate blocking based on region or country.
    """
    geo_errors = [
    # This list was made after having manually checked numerous geoblocked
    # pages. Contains the most common strings found.
                ...
                "Your country or region is blocked from accessing our site"
                ]
                
    geo_tag = ''
    # Searching for the Cloudflare's region blocked error
    if re.search("<span class=\"cf-error-code\">1009</span>", body):
        geo_tag = ":GEO_BLOCKED-1009"
    # Searching for the SonicWall filter
...
def get_abuse_block(body):
    """
    Returns a tag of abuse-blocked if the HTML body contains any regexes
    which indicate blocking due to abusive traffic.
    """
    other_errors = [
                    "Request forbidden by administrative rules.",
                    ]
    
    # Strings were made after reading Cloudflare's documentation and forums
    ip_errors = [
    ...
    elif re.search("Access Denied - Sucuri Website Firewall", body):
        if re.search("DDOS attempt was blocked", body):
            abuse_tag = ":DDOS_attempt_Sucuri"
        elif re.search("Bad bot access attempt", body):
            abuse_tag = ":badbot_attempt_Sucuri"
        else:
            abuse_tag = ":Sucuri"

    elif re.search("This website is using a security service to protect itself", body):
        abuse_tag = ":CLOUDFLARE_Security"
        
    elif re.search("Our systems have detected unusual traffic from your computer network", body):
        abuse_tag = ":Google_unusual_traffic"

    elif re.search("Request unsuccessful. Incapsula incident", body):
        abuse_tag = ": Incapsula incident"
        
    elif re.search("security firewall is blocking your access", body):
        abuse_tag = ": Security_firewall"
    
    elif re.search("Protected by WireFilter", body):
        abuse_tag = ": Protected by WireFilter"
    
    elif re.search("DDoS protection by Cloudflare", body):
        abuse_tag = ":DDoS_protection_by_Cloudflare"
...
def response(status, headers, body):
    ...
    doing the blocking. In addition to that, the string contains a tag of
    abuseblocking or geoblocking (or both). These tags begin with a colon ":"
    after the blocktype.
    
    The first component is False if the status and body is
    categorized, and the body contained none of the regex we searched for.
    In this case, the status converted to a string, and returned with a
    'NONBLOCK' blocktype.
    """

    # List for various sorts of unsupported browser messages
    browser_errors = [
    ...
    # Get geoblocked and abuseblocked tags, if they exist
    ...
    if status == 400:
            if re.search("The plain HTTP request was sent to HTTPS port", body):
                    blocktype = "400-HTTP_sent_to_HTTPS_port"
            elif re.search("Your browser sent an invalid request", body):
                    blocktype = "400-browser_sent_invalid_req"
    ...
    if status == 502:
        if re.search("The delicious site is temporarily offline while we move servers", body):
                blocktype = "502-Delicius-temp"
        if status == 503:
    ...
    if blocktype:
        # Blocktype exists, i.e., Some regex was found in the HTML body. Could also
        # have a geoblocked or abuseblocked tag in addtion to the blocktype.
        if (geo_tag == '' and abuse_tag != ''):
            ...
            return True, blocktype
                ...
                # Extremely rare case when the HTML content shows both abuseblock
                # and geoblocked, in addition to satisfying David's regexes.
            return True, blocktype+"$BOTH$"+geo_tag+abuse_tag
    else:
        # No old regexes (David's) were found in the HTML body, however could
        # still have a geoblocked or abuseblocked tag.
        if (geo_tag == '' and abuse_tag != ''):
            return True, str(status)+"-UNCAUGHT"+abuse_tag
        elif (geo_tag != '' and abuse_tag == ''):
            return True, str(status)+"-UNCAUGHT"+geo_tag
        elif (geo_tag == '' and abuse_tag == ''):
                # The only instance when the HTML body contains NONE of the regexes,
                # neither the old ones (David's), nor the geoblocked/abuseblocked
                # ones.
